...
2. **Run the scaffold script** — replaces all tokens and renames `tiers.php → <slug>.php`
   ...
   |---|---|---|
   | `tiers` | lowercase slug = text-domain = i18n domain | `restock` |
   | `Tiers` | PSR-4 PHP namespace | `Restock` |
   | `TIERS` (in `define()`) | uppercased namespace | `RESTOCK` |
   | `tiers_` | option/meta prefix (slug, dashes→underscores) | `restock_` |
   | `Tiers` / `Volume and quantity-based tiered pricing for WooCommerce` / `Volume and quantity-based tiered pricing for WooCommerce` | name + descriptions | … |
...
- **Bootstrap** (`tiers.php`): PHP/WC guards, HPOS + cart-blocks compat, `plugins_loaded`
  boot, `do_action('tiers/booted')` (the hook a PRO companion extends).
