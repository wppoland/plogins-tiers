...
* Fully translatable: ships the `plogins-tiers.pot` template (text domain `plogins-tiers`), with Polish, German and Spanish delivered by the WordPress.org language pack
...
Tiers is developed in the open. The PHP, JS, and CSS you install are the same files in the repository, nothing is minified or generated by a build step. Read the code, file a bug, or send a patch at [github.com/wppoland/plogins-tiers](https://github.com/wppoland/plogins-tiers).
...
= 1.0.16 =
* The translation template was regenerated. 4 strings added to the plugin since the template was last built were missing from it, so no translator could reach them in any language, and 1 strings the plugin no longer uses have been dropped. Nothing you see changes; what a translator can see does.
